...
namespace BinFileUtils
{
    ...
    std::string BinFile::readString()
    {
        ...
        const uint8_t* dataArray = static_cast<const uint8_t*>(addr) + pos;
        u_int64_t remainingSize = size - pos;

        const uint8_t* endOfString = std::find(dataArray + pos, dataArray + remainingSize, 0);
        bool endOfStringFound = (endOfString != (dataArray + remainingSize));

        std::string str;
        if (endOfStringFound) {
            str = std::string(dataArray, endOfString);
            pos += (endOfString - dataArray) + 1;
        }
